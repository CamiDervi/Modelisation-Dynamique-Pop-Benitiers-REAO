...
load("Q:/Bénitiers Tahiti/R_REAO/OUTPUT/ENVIRONNEMENTS/Last_Enviro_03-07_v3.RData")
...
scenarios
  ...
  for (bassin in 1:1){
    ...
    for (s in scenarios) {
      ...
      # ESTIMATIONS
      N_MEDIANE_Est <- apply(result[5:21, , PU_list, "MEDIANE", s],c(2,3),sum)   # Ab/PU/campagne
      N_Q5_Est <- apply(result[5:21, , PU_list, "Q5", s],c(2,3),sum)
      N_Q95_Est <- apply(result[5:21, , PU_list, "Q95", s],c(2,3),sum)
    ...
    for (bassin in 1:2){
      ...
      campagnes_obs <- unique(Obs_PU$CAMPAGNE)
        ...
        for (s in scenarios) {
          ...
          # ESTIMATIONS
          N_MEDIANE_Est <- apply(result[13:21, , PU_list, "MEDIANE", s],c(2,3),sum)
          N_Q5_Est <- apply(result[13:21, , PU_list, "Q5", s],c(2,3),sum)
          N_Q95_Est <- apply(result[13:21, , PU_list, "Q95", s],c(2,3),sum)
  ...
  # 3 - COMPARAISON DES STRUCTURES DE TAILLE
    ...
    #correspondance <- correspondance[(i+1):(i+1)]
    #cor.time.camp <- cor.time.camp[(i+1),, drop = FALSE] 
        ...
        for (s in scenarios){
          for (c in cor.time.camp[,"CAMPAGNE"]){
            for (sg in as.character(Nstage[-c(1:4)])){
              ...
              SS.res[s] <- SS.res[s] + ((sum(result[sg,cor.time.camp[cor.time.camp[,"CAMPAGNE"]==c,"TIME"],as.character(unique(Tab_valid$PU)),"MEDIANE",s])/sum(Tab[Tab$PU%in%as.character(unique(Tab_valid$PU)),"AREA"]))
    ...
    # 3.2 - Structure de taille simulées versus observées
      ...
      #s.select = "sf_6_b_4e-04_e_0.2_d_4e-04_Peche_chair_1_Peche_viv_1_T_capture_NA"
      ...
      for (bassin in 1:1) { 
        ...
        for (i in seq(1, n_campagnes, by = n_panels_per_page)) {
          ...
          for (c in campagnes_this_page) {
            ...
            # --- Valeurs Simulation ---
            sim_vals <- apply(
              result[5:21, time.sim, as.character(unique(Tab_valid$PU)), "MEDIANE", s.select],1,sum) / AIRE_PU_VALID
